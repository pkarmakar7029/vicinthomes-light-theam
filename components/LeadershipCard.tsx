...
export default function LeadershipCard({ leader }: Props) {
  return (
    <motion.article
      ...
      className="lux-card overflow-hidden flex flex-col bg-[color:var(--background-main)]/90 hover:border-[color:var(--accent-brass)] hover:shadow-[0_20px_50px_rgba(42,42,42,0.12)] transition-all duration-300"
    >
      <div className="flex flex-col items-center px-6 pt-8 pb-7 text-center">
        ...
        <p className="mt-3 text-xs sm:text-sm text-neutral-700">
          With a two-decade perspective on Indian luxury real estate, {leader.name.split(" ")[0]} leads
          Vicint&apos;s design and development thesis across key markets.
        </p>
      </div>
    </motion.article>
  );
